...
struct Payload {
    // 枚举的成员变量可以提升为整型，参与整型的计算，即使枚举本身没有定义算术操作
    // 因为枚举类型的变量只能取枚举列表中的值，不能取其他整型值（在没有强制类型转换的情况下），即使它们在底层表示上是相同的
    // 我们可以显式地给枚举成员变量赋值，也可以不赋值，默认从 0 开始递增
    // enum Kind { AsInt = 0, AsDouble = 1, AsText = 3 } kind;
    // 还可以不定义类型名，直接在枚举列表中定义枚举成员变量并直接使用
    // enum  AsInt, AsDouble, AsText };
